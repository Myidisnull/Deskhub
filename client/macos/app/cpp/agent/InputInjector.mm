...
//
// BỐ CỤC
//   SourceRect()      — rect nguồn theo ĐIỂM, toàn cục (có cache 200ms).
//   TargetHasFocus()  — chốt an toàn: ứng dụng đích có đang foreground không.
//   Apply()           — cổng vào duy nhất, gác ba cơ chế an toàn rồi phân loại event.
...
//   kênh điều khiển tự khoá chính nó — xem agent/LocalInputMonitor.h.
//
...
//
// LIÊN QUAN: agent/InputInjector.h (ba cơ chế an toàn + ánh xạ toạ độ),
//            input/MacKeyMap.h (VK → keycode), agent/LocalInputMonitor.h
// =============================================================================
...
// Rect nguồn được hỏi lại nhiều nhất mỗi ngần này. Chuột di chuyển sinh hàng trăm
// event mỗi giây còn cửa sổ thì hiếm khi bị kéo — hỏi WindowServer mỗi event là
// lãng phí thấy rõ, mà 200ms trễ khi kéo cửa sổ thì không ai nhận ra.
...
bool InputInjector::Init(const CaptureTarget& target) {
    ...
    if (!SourceRect(x, y, w, h)) {
        LOGE("[Input] Source %u has no bounds — input disabled for it.", target.id);
        return false;
    }
    return true;
}

// Rect nguồn theo ĐIỂM, gốc trên-trái toàn cục. Cache kRectCacheUs.
bool InputInjector::SourceRect(double& x, double& y, double& w, double& h) {
    ...
    if (target_.isDisplay) {
        const CGRect r = CGDisplayBounds(CGDirectDisplayID(target_.id));
        ...
        if (CFArrayGetCount(arr) > 0) {
            ...
            if (CGRectMakeWithDictionaryRepresentation(
                ...
                rectW_ = r.size.width;
                rectH_ = r.size.height;
                NSNumber* pid = d[(__bridge NSString*)kCGWindowOwnerPID];
                ownerPid_ = pid ? int32_t(pid.intValue) : 0;
                ok = true;
            }
        }
        CFRelease(arr);
        if (!ok) return false; // cửa sổ đã đóng
    }
...
// Cơ chế an toàn 1 — xem đầu InputInjector.h.
bool InputInjector::TargetHasFocus() {
    if (target_.isDisplay) return true; // cả màn hình đã được chia sẻ
    if (!ownerPid_) {
        double x, y, w, h;
        if (!SourceRect(x, y, w, h)) return false;
    }
    if (!ownerPid_) return false;
    NSRunningApplication* front = [[NSWorkspace sharedWorkspace] frontmostApplication];
    return front && int32_t(front.processIdentifier) == ownerPid_;
}

bool InputInjector::FocusTarget() {
    if (target_.isDisplay) return true;
    double x, y, w, h;
    if (!SourceRect(x, y, w, h)) return false;
    if (!ownerPid_) return false;

    NSRunningApplication* app =
        [NSRunningApplication runningApplicationWithProcessIdentifier:pid_t(ownerPid_)];
    if (!app) return false;
    if (app.active) return true;
    // activateWithOptions phải chạy trên main thread; AgentLoop gọi từ thread Recv.
    // async chứ không sync: kéo một app lên trước có thể mất vài trăm ms (nó phải
    // vẽ lại), và chặn thread Recv ngần ấy là bỏ lỡ gói UDP.
...
// Cổng vào duy nhất. Ba lớp gác trước khi bơm bất cứ thứ gì:
//   1. Bật/tắt (người chia sẻ chọn chế độ chỉ-xem).
//   2. Chốt foreground (input không được rơi sang ứng dụng khác của chủ máy).
//   3. Host thắng (người ngồi máy vừa dùng chuột/phím thật).
void InputInjector::Apply(const deskhub::InputEvent& e) {
    ...
    if (!TargetHasFocus()) {
        if (hadFocus_) {
            hadFocus_ = false;
            LOGI("[Input] Shared window is not frontmost — remote input ignored "
                 "until it is clicked on this Mac.");
            ReleaseAll();
        }
        ++skipped_;
        return;
    ...
    if (!hadFocus_) {
        hadFocus_ = true;
        LOGI("[Input] Shared window is frontmost again — remote input active.");
    }
