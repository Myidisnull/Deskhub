...
void ClientLoop::DecodeThread() {
    ...
    for (;;) {
        ...
        // (Xem VtDecoder::lastRenderedPtsUs về caveat "enqueue vs lên màn hình", và deskhub/control/ClockOffset.h về vì sao là bộ lọc min.)
...
void ClientLoop::NetThread() {
    ...
    // Cỡ MÀN HÌNH thiết bị (pixel). Host co luồng cho vừa — một iPhone không có việc
    // gì phải nhận 3024×1964 từ một MacBook Retina rồi vẽ xuống khung bé hơn ba lần.
    // 0 = tầng bridge không lấy được cỡ; host khi đó chỉ dùng trần của riêng nó.
