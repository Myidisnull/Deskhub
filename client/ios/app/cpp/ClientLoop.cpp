...
void ClientLoop::NetThread() {
    ...
    // Host vừa nhớ máy này. Giữ lại để UI lấy đi cất vào Keychain — token chỉ đi
    ...
    deskhub::Hello hello;
    // clientId ỔN ĐỊNH theo cài đặt (Credentials.swift sinh một lần rồi cất
    // Keychain). Trước GĐ10 nó lấy từ đồng hồ vì chỉ cần phân biệt hai client; giờ
    // host khoá danh sách thiết bị tin cậy theo trường này, nên clientId đổi mỗi lần
    // mở app đồng nghĩa với việc deviceToken không bao giờ khớp lại được.
    // Lùi về đồng hồ nếu caller quên truyền — mất tính năng nhớ thiết bị, nhưng vẫn
    // kết nối được bằng mật khẩu.
