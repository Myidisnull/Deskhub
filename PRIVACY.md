...
_Effective date: August 13, 2026 — Version 1.6_
...
| Clipboard text (desktop only, only while the clipboard-sync toggle is on and a session is active) | Making text copied on one machine pastable on the other | Sent directly between your devices — unencrypted, like the rest of the traffic — capped at 32 KiB per copy; only plain text, never images or files | Never stored by Deskhub; lives only in each machine's normal system clipboard |
