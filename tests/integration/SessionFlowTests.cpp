...
void TestAViewerLearnsWhatTheHostCannotDo() {
    ...
    Check(QuerySources(HostAddr(port), sources, kTestPasscode, &caps),
    ...
    Check(QuerySources(HostAddr(deskPort), sources, kTestPasscode, &caps),
        "a desktop answers the same query");
    Check(caps.acceptsInput, "and a host that does take input is not mistaken for a phone");
    Check(!caps.terminal, "develop hosts do not advertise a shared terminal");
