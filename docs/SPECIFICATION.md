...
| C-10 | Connect, then choose | Connect only authenticates: it dials the host, settles pairing or the passcode (S-2), and asks what the host shares. Until it succeeds the page is the address, port, passcode and name fields, Connect, and the device list — nothing else. Once the host answers those give way to its address, a **Disconnect** control, the live line of V-7, and one button each for *Remote desktop — view its screen*, *Terminal — open a shell* and *File transfer — send files to it*, each enabled only if the host shares that capability. Disconnect, or editing the address, port or passcode, returns the page to the first state. Opening a session reuses the pairing just settled, so nobody at the host is asked a second time. |
