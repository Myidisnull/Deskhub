...
| C-8 | Open a shell | On desktop, the Client page has a *Terminal — open a shell* tick beside *Desktop*; Connect opens whatever is ticked, reusing the same address, port, passcode and name fields. On Android and iOS it is a *Terminal — open a shell* button beside Connect, using the same fields. The shell opens in its own window — grid, scrollback, status line, and on phones an extra key row (Esc, Tab, latching Ctrl/Alt, arrows, ^C) — and that window states why a shell could not open (wrong passcode, refused, unreachable). Watching the screen and running a shell at the same time is normal. Every client asks the host what it shares before opening anything: against a host with no terminal — a phone or tablet, or a desktop not sharing one — no terminal window is opened at all and the connect page says so instead. |
| C-9 | Send files | Every client can send files to a host that is taking them. On desktop, the Client page has a *File transfer — send files to it* tick beside *Desktop* and *Terminal*; Connect opens a **Send files** window using the same address, port, passcode and name fields. A viewer window can also send mid-session, from the same window that shows the screen. On Android and iOS it is a *File transfer — send files to it* button beside Connect, and a **Files** button in the session controls; files are picked from the system photo picker or the system file browser, and a copy is staged in the app's own cache before it is sent. One batch at a time: while one is running the pickers are disabled and a second offer is refused as busy. Progress names the file being sent, its place in the batch and the share done, and the transfer can be stopped at any point. When it settles, every file in the batch is listed as sent or not sent, with the reason. Every client asks the host what it shares before opening anything: against a host that is not taking files, no window opens and the connect page says so instead. |
...
- Receiving files on a phone or tablet — mobile devices send files to a host but never
  take them (P-4). Remote printing.
