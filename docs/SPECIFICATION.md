...
| C-8 | Open a shell | On Windows and Linux, the Client page has a *Terminal — open a shell* tick beside *Desktop*; Connect opens whatever is ticked, reusing the same address, port, passcode and name fields. On Android it is a *Terminal — open a shell* button beside Connect, using the same fields; on macOS and iOS the button follows C-10 and enables only after Connect has learned the host shares a terminal. The shell opens in its own window — grid, scrollback, status line, and on phones an extra key row (Esc, Tab, latching Ctrl/Alt, arrows, ^C) — and that window states why a shell could not open (wrong passcode, refused, unreachable). Watching the screen and running a shell at the same time is normal. Every client learns what the host shares before opening anything: against a host with no terminal — a phone or tablet, or a desktop not sharing one — no terminal window is opened at all; the connect page says so on Windows, Linux and Android, and on macOS and iOS the button simply stays disabled. |
| C-9 | Send files | Every client can send files to a host that is taking them. On Windows and Linux, the Client page has a *File transfer — send files to it* tick beside *Desktop* and *Terminal*; Connect opens a **Send files** window using the same address, port, passcode and name fields. On Android it is a *File transfer — send files to it* button beside Connect; on macOS and iOS the button follows C-10. On Android and iOS files are picked from the system photo picker or the system file browser, and a copy is staged in the app's own cache before it is sent. One batch at a time: while one is running the pickers are disabled and a second offer is refused as busy. Progress names the file being sent, its place in the batch and the share done, and the transfer can be stopped at any point. When it settles, every file in the batch is listed as sent or not sent, with the reason. Every client learns what the host shares before opening anything: against a host that is not taking files, no window opens; the connect page says so on Windows, Linux and Android, and on macOS and iOS the button simply stays disabled. |
| C-10 | Connect, then choose (macOS, iOS) | On macOS and iOS the Connect button only authenticates: it dials the host, settles pairing or the passcode (S-2), and asks what the host shares. The *Remote desktop — view its screen*, *Terminal — open a shell* and *File transfer — send files to it* buttons stay disabled until that succeeds, and each then enables only if the host shares that capability; a note under the buttons says they light up with what the machine shares. Editing the address, port or passcode disables them again until the next Connect. Opening a session reuses the pairing just settled, so nobody at the host is asked a second time. |
...
| V-7 | Connection health | The connect page carries the reading, not the session window: once a host has answered, that page shows its address, a **Disconnect** control (V-5) and a live line saying it is connected, with the ping beside it, going red the moment the host stops answering. The reading comes from the same once-a-second probe that fills the device list, so it is there before a session is opened and stays there while several are. A session window that loses its host still says it is reattaching (V-8). |
