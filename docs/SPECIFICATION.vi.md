...
| C-8 | Mở shell | Trên Windows và Linux, trang Client có ô tích *Terminal — open a shell* cạnh *Desktop*; bấm Connect sẽ mở đúng những gì đã tích, dùng lại chính các ô địa chỉ, cổng, passcode và tên. Trên Android đó là nút *Terminal — open a shell* cạnh Connect, dùng chung các ô đó; trên macOS và iOS nút này theo luồng C-10 và chỉ bật sau khi Connect biết host có chia sẻ terminal. Shell mở trong cửa sổ riêng — lưới ký tự, cuộn lại lịch sử, dòng trạng thái, và trên điện thoại có thêm hàng phím phụ (Esc, Tab, Ctrl/Alt giữ trạng thái, mũi tên, ^C) — và cửa sổ đó nêu rõ vì sao shell không mở được (sai passcode, bị từ chối, không tới được máy). Vừa xem màn hình vừa chạy shell là chuyện bình thường. Mọi client đều biết host chia sẻ những gì trước khi mở bất cứ thứ gì: với host không có terminal — điện thoại, máy tính bảng, hoặc một desktop không chia sẻ terminal — không cửa sổ terminal nào được mở ra cả; trên Windows, Linux và Android trang kết nối báo lý do, còn trên macOS và iOS nút này đơn giản là không bật. |
| C-9 | Gửi tệp | Mọi client đều có thể gửi tệp tới host đang nhận tệp. Trên Windows và Linux, trang Client có ô tích *File transfer — send files to it* cạnh *Desktop* và *Terminal*; bấm Connect sẽ mở cửa sổ **Send files** dùng lại chính các ô địa chỉ, cổng, passcode và tên. Trên Android đó là nút *File transfer — send files to it* cạnh Connect; trên macOS và iOS nút này theo luồng C-10. Trên Android và iOS, tệp được chọn từ bộ chọn ảnh của hệ thống hoặc trình duyệt tệp của hệ thống, và một bản sao được chuẩn bị trong cache riêng của app trước khi gửi. Mỗi lần một lô: khi một lô đang chạy thì các bộ chọn bị khóa và lời chào thứ hai bị từ chối vì đang bận. Tiến độ nêu tên tệp đang gửi, vị trí của nó trong lô và phần đã xong, và có thể dừng lượt truyền bất cứ lúc nào. Khi lô kết thúc, từng tệp được liệt kê là đã gửi hay chưa, kèm lý do. Mọi client đều biết host chia sẻ những gì trước khi mở bất cứ thứ gì: với host không nhận tệp, không cửa sổ nào được mở ra cả; trên Windows, Linux và Android trang kết nối báo lý do, còn trên macOS và iOS nút này đơn giản là không bật. |
| C-10 | Kết nối rồi mới chọn (macOS, iOS) | Trên macOS và iOS, nút Connect chỉ làm việc xác thực: nó gọi tới host, hoàn tất ghép đôi hoặc passcode (S-2), và hỏi host đang chia sẻ những gì. Các nút *Remote desktop — view its screen*, *Terminal — open a shell* và *File transfer — send files to it* bị khóa cho tới khi bước đó thành công, và sau đó mỗi nút chỉ bật nếu host có chia sẻ khả năng tương ứng; một dòng ghi chú dưới các nút nói rằng chúng sáng lên theo những gì máy kia chia sẻ. Sửa địa chỉ, cổng hay passcode sẽ khóa các nút lại cho tới lần Connect kế tiếp. Mở một phiên dùng lại chính lần ghép đôi vừa xong, nên không ai ở phía host bị hỏi lần thứ hai. |
...
| V-7 | Sức khỏe kết nối | Số đo nằm ở trang kết nối chứ không phải cửa sổ phiên: khi một host đã trả lời, trang đó hiện địa chỉ của nó, nút **Ngắt kết nối** (V-5) và một dòng trực tiếp báo đang kết nối, kèm ping bên cạnh, chuyển đỏ ngay khi host ngừng trả lời. Số đo đến từ chính nhịp dò mỗi giây một lần nuôi danh sách thiết bị, nên nó có mặt trước khi mở phiên và ở lại trong lúc nhiều phiên đang chạy. Cửa sổ phiên mất host vẫn báo đang nối lại (V-8). |
