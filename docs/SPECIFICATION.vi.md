...
| C-8 | Mở shell | Trên desktop, trang Client có ô tích *Terminal — open a shell* cạnh *Desktop*; bấm Connect sẽ mở đúng những gì đã tích, dùng lại chính các ô địa chỉ, cổng, passcode và tên. Trên Android và iOS đó là nút *Terminal — open a shell* cạnh Connect, dùng chung các ô đó. Shell mở trong cửa sổ riêng — lưới ký tự, cuộn lại lịch sử, dòng trạng thái, và trên điện thoại có thêm hàng phím phụ (Esc, Tab, Ctrl/Alt giữ trạng thái, mũi tên, ^C) — và cửa sổ đó nêu rõ vì sao shell không mở được (sai passcode, bị từ chối, không tới được máy). Vừa xem màn hình vừa chạy shell là chuyện bình thường. Mọi client đều hỏi host xem nó chia sẻ những gì trước khi mở bất cứ thứ gì: với host không có terminal — điện thoại, máy tính bảng, hoặc một desktop không chia sẻ terminal — không cửa sổ terminal nào được mở ra cả, thay vào đó trang kết nối báo lý do. |
| C-9 | Gửi tệp | Mọi client đều có thể gửi tệp tới host đang nhận tệp. Trên desktop, trang Client có ô tích *File transfer — send files to it* cạnh *Desktop* và *Terminal*; bấm Connect sẽ mở cửa sổ **Send files** dùng lại chính các ô địa chỉ, cổng, passcode và tên. Cửa sổ viewer cũng gửi được giữa phiên, ngay từ cửa sổ đang hiện màn hình. Trên Android và iOS đó là nút *File transfer — send files to it* cạnh Connect, và nút **Files** trong bảng điều khiển phiên; tệp được chọn từ bộ chọn ảnh của hệ thống hoặc trình duyệt tệp của hệ thống, và một bản sao được chuẩn bị trong cache riêng của app trước khi gửi. Mỗi lần một lô: khi một lô đang chạy thì các bộ chọn bị khóa và lời chào thứ hai bị từ chối vì đang bận. Tiến độ nêu tên tệp đang gửi, vị trí của nó trong lô và phần đã xong, và có thể dừng lượt truyền bất cứ lúc nào. Khi lô kết thúc, từng tệp được liệt kê là đã gửi hay chưa, kèm lý do. Mọi client đều hỏi host xem nó chia sẻ những gì trước khi mở bất cứ thứ gì: với host không nhận tệp, không cửa sổ nào được mở ra cả, thay vào đó trang kết nối báo lý do. |
...
| P-4 | Android / iOS | Chia sẻ ở chế độ **chỉ xem**: thiết bị phát màn hình và lặng lẽ bỏ qua mọi gói điều khiển, vì cả hai hệ điều hành đều không cho ứng dụng bơm thao tác vào toàn hệ thống. Nó cũng không chia sẻ terminal và không nhận tệp, và nó trả lời viewer những điều đó, nên không client nào mở cửa sổ terminal nhắm vào điện thoại (C-8) và các client desktop có thể nói trước rằng ô điều khiển sẽ không có tác dụng (C-3). Toàn bộ màn hình được chia sẻ như một nguồn duy nhất, nên bộ chọn màn hình, chia sẻ nhiều màn hình và dừng từng màn hình (H-1, H-2, H-3, H-5) không áp dụng. Xoay thiết bị thì luồng xoay theo: hình người xem thấy vẫn đúng chiều, và cửa sổ của họ tự chỉnh lại theo hình dạng mới (V-1). Giao diện phiên ưu tiên cảm ứng: cử chỉ trackpad, nút phóng to, thanh phím tắt, bàn phím ảo, nút đổi màn hình và **End**. |
...
- Nhận tệp trên điện thoại hay máy tính bảng — thiết bị di động gửi tệp tới host nhưng
  không bao giờ nhận tệp (P-4). In từ xa.
