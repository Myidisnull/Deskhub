...
|---|---|---|---|
| Encode | `evt=sum` (per source, 1 s) | `enc_lat_ms=<avg>/<max>` | **The encoder's real latency**: capture timestamp → the moment the compressed NAL comes back out, measured entirely on the host clock. Added 2026-07-30 because it was the largest blind spot in the whole chain. Do not confuse it with `enc_ms` below — all three encoders compress **asynchronously**, so `enc_ms` only times the *submit* call, and how long a frame then sits inside the encoder's pipeline was never counted. This matters more than any other single number: a pipeline four frames deep at 60 fps is 67 ms of **constant** latency, and the client's `e2e_ms` subtracts constants by construction (it is a min-filtered *jitter* metric — see the e2e section). A healthy value is roughly one frame interval; several frame intervals means the encoder is running deep and the user feels lag that no client-side counter can see. |
| Capture | `evt=sum` (per source, 1 s) | `cap_idle` | How many times ScreenCaptureKit **called us but said "no new content"** (`SCFrameStatus != Complete`). Read it next to `capture N fps` on the `[Agent]` line — alone, that fps number cannot tell two very different faults apart: a high `cap_idle` means macOS is calling steadily and genuinely believes the screen is static, while `cap_idle ≈ 0` *together with* a low capture rate means macOS **stopped calling us at all** — the frame pool ran out of buffers (see `queueDepth` in `capture/ScreenCapture.mm`) or the stream died. Those need opposite fixes, and before 2026-07-30 they were indistinguishable in the log. |
