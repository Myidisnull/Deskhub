...
void TestLateJoinerGetsAKeyframe() {
    ...
    Check(s->state() == HostSession::State::Streaming, "and the source keeps streaming");
}
...
void TestSessionEndsOnlyWhenTheLastViewerLeaves() {
    ...
    Check(s->state() == HostSession::State::Streaming, "one BYE does not end the stream");
    Check(rec.disconnects == 0, "and does not report a disconnect");

    s->HandlePacket(ByeFor(sid), kT0, kAlice);
    Check(s->state() == HostSession::State::Idle, "the last BYE ends it");
...
void TestNonsenseFromAViewerIsIgnored() {
    ...
    Check(s->viewerCount() == 1 && s->state() == HostSession::State::Streaming,
