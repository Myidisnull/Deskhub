...
size_t BuildPairingHello(std::span<uint8_t> out, const PairingHello& m) {
    const size_t nameLen = m.clientName.size() < kMaxPairedNameBytes ? m.clientName.size()
                                                                     : kMaxPairedNameBytes;
    const size_t payload = kFingerprintBytes + 1 + nameLen;
    const size_t total = WriteCommon(out, MsgType::PairingHello, 0, Chan::Control, 0, payload);
    if (!total) return 0;
    uint8_t* p = out.data() + kCommonHeaderSize;
    std::memcpy(p, m.fingerprint.bytes.data(), kFingerprintBytes);
    p[kFingerprintBytes] = uint8_t(nameLen);
    if (nameLen) std::memcpy(p + kFingerprintBytes + 1, m.clientName.data(), nameLen);
    return total;
}

size_t BuildPairingResult(std::span<uint8_t> out, const PairingResult& m) {
    const size_t total = WriteCommon(out, MsgType::PairingResult, 0, Chan::Control, 0, 1);
    if (!total) return 0;
    out[kCommonHeaderSize] = uint8_t(m.code);
    return total;
}

std::optional<PairingHello> ParsePairingHello(std::span<const uint8_t> payload) {
    if (payload.size() < kFingerprintBytes + 1) return std::nullopt;
    PairingHello m;
    std::memcpy(m.fingerprint.bytes.data(), payload.data(), kFingerprintBytes);
    if (IsZero(m.fingerprint)) return std::nullopt;
    const size_t nameLen = payload[kFingerprintBytes];
    if (nameLen > kMaxPairedNameBytes) return std::nullopt;
    if (payload.size() < kFingerprintBytes + 1 + nameLen) return std::nullopt;
    if (nameLen) m.clientName.assign(reinterpret_cast<const char*>(payload.data() + kFingerprintBytes + 1),
        nameLen);
    return m;
}
...
std::optional<PairingResult> ParsePairingResult(std::span<const uint8_t> payload) {
    if (payload.empty()) return std::nullopt;
    if (payload[0] > uint8_t(PairingResultCode::Disabled)) return std::nullopt;
    PairingResult m;
    m.code = PairingResultCode(payload[0]);
    return m;
}
