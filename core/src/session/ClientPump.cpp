...
ClientPump::ClientPump(ClientPumpCallbacks cb, diag::ClientDiag& diag)
    : cb_(std::move(cb)),
      diag_(diag),
      session_(MakeSessionCallbacks()),
      linkStats_(0) {}

ClientCallbacks ClientPump::MakeSessionCallbacks() {
    ClientCallbacks sc;
...
void ClientPump::Start(const ClientPumpConfig& cfg, uint64_t nowUs) {
    cfg_ = cfg;
...
void ClientPump::EnsureReassembler() {
    if (reasm_) return;
    ...
    reasm_->onFrameDrop = [this](const Reassembler::FrameDropInfo& d) {
        char drop[diag::ClientDiag::kFrameDropBufBytes];
        LogWarn(diag::ClientDiag::FormatFrameDrop(drop, sizeof(drop), d));
    };
}
...
void ClientPump::RequestKeyframe(const char* reason, uint64_t nowUs) {
...
void ClientPump::PlanNacks(uint64_t nowUs) {
...
void ClientPump::PollFrames(uint64_t nowUs) {
    if (!reasm_) return;
...
void ClientPump::Report(uint64_t nowUs) {
    ...
    Log(diag::ClientDiag::FormatStatus(line_, sizeof(line_), hms.c_str(), w,
...
bool ClientPump::Tick(uint64_t nowUs) {
    session_.Tick(nowUs);
    if (session_.state() == ClientSession::State::Dead) return false;

    if (linkStats_.Due(nowUs)) Report(nowUs);
    return true;
}

void ClientPump::CountLoopBusy(uint64_t startedUs, uint64_t nowUs) {
