...
//
// LIÊN QUAN
//   deskhub/protocol/ByteOrder.h — lớp dịch byte bên dưới
...
enum class MsgType : uint8_t {
    ...
    // 0x07–0x0A từng là Discover/Announce/AuthChallenge/AuthResponse — đã bỏ
    // (app chỉ dùng trong LAN tin cậy). Giữ trống để không đổi giá trị các entry sau.
    VideoPacket = 0x10,
    ...
    Reconfig = 0x34,
    SetFocus = 0x35,      // client báo đang/thôi điều khiển nguồn này (false → host nhả phím)
...
// Một host chia sẻ nhiều MÀN HÌNH cùng lúc (máy nhiều monitor). Mỗi màn hình là
// một "nguồn" có sourceId riêng, và mỗi cặp (client, nguồn) là một PHIÊN ĐỘC LẬP
// với sessionId riêng — không nhét streamId vào header video/input. Nhờ vậy kênh
// video, input, FEC, congestion control giữ nguyên y hệt trường hợp một nguồn, và
// mỗi nguồn tự điều chỉnh bitrate / xin IDR theo tình trạng riêng (mỗi nguồn một
// encoder). (Share theo TỪNG CỬA SỔ — SourceKind::Window — đã bỏ 2026-07-27: app
// tập trung thuần remote desktop, nguồn chỉ còn là màn hình.)
inline constexpr size_t kMaxSources = 8;
inline constexpr size_t kMaxSourceNameBytes = 64; // tên màn hình, UTF-8, cắt bớt

struct SourceInfo {
    uint8_t sourceId = 0;
    uint16_t width = 0;
    uint16_t height = 0;
    std::string name; // tên màn hình (UTF-8), chỉ để hiển thị
};
...
// `focused` = 1: người dùng đang xem/điều khiển nguồn này. = 0: rời đi (đổi nguồn,
// app xuống nền) → host NHẢ HẾT PHÍM đang giữ của phiên này, chống kẹt phím khi gói
// key-up không bao giờ tới nữa. (Vai trò cũ "đưa cửa sổ nguồn lên foreground" đã bỏ
// cùng share-theo-cửa-sổ 2026-07-27 — với nguồn màn hình không có gì để raise.)
