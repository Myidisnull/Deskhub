...
void TestPairingRequestsSurviveASmallWindow() {
    std::printf("[sharinghost] a pairing request never falls off the back of a small window...\n");
    SharingHost loop;
...
void TestPairingQueueCannotGrowForever() {
    std::printf("[sharinghost] a queue nobody drains stops growing instead of eating memory...\n");
    SharingHost loop;
