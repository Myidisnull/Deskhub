...
std::filesystem::path DefaultTransferDir() {
    if (!AppDataDirRef().empty()) {
        const std::u8string wide(AppDataDirRef().begin(), AppDataDirRef().end());
        return std::filesystem::path(wide) / kTransferDirName;
    }
    const std::filesystem::path home = HomePath();
    if (home.empty()) return {};
    return home / deskhub::brand::kDataDirName / kTransferDirName;
}
