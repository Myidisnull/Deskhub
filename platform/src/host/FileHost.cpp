...
bool FileHost::Start(UdpSocket& sock, const std::filesystem::path& dir,
...
std::vector<FileHost::Outbound> FileHost::TakeOutbox() {
    std::vector<Outbound> records;
    records.swap(outbox_);
    return records;
}

void FileHost::FlushOutbox() {
    UdpSocket* sock = nullptr;
    std::vector<Outbound> outbound;
    {
        const std::lock_guard<std::mutex> lock(mutex_);
...
FileHost::Peer* FileHost::PeerFor(const NetAddr& from) {
    ...
    hooks.send = [this, raw](std::span<const uint8_t> message) {
        outbox_.push_back(Outbound{raw->addr, {message.begin(), message.end()}});
    };
    ...
    peer->receiver->SetPeer(deskhub::TransferPeer{peer->endpoint, peer->name});
...
void FileHost::RefuseOffer(const NetAddr& from, std::span<const uint8_t> message) {
    const auto header = deskhub::ParseCommonHeader(message);
    ...
    const size_t n = deskhub::BuildFileAccept(out, refusal);
    if (!n) return;
    out.resize(n);
    if (sock_) SendFileMessage(*sock_, from, out);
}

void FileHost::HandleDatagram(const NetAddr& from, std::span<const uint8_t> datagram) {
    if (!running_.load(std::memory_order_acquire)) {
        if (sock_) RefuseOffer(from, datagram);
        return;
    }
    ...
    std::vector<std::vector<uint8_t>> messages;
    {
        ...
        Peer* peer = PeerFor(from);
        if (peer == nullptr) return;
        FeedFileDatagram(peer->stream, datagram, messages);
    }
