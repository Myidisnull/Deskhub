...
bool TerminalHost::Start(UdpSocket& sock, std::string shell, std::string passcode,
    TerminalHostCallbacks callbacks, std::function<bool(uint64_t addrPacked)> peerAuthenticated) {
...

    {
        ...
        peerAuthenticated_ = std::move(peerAuthenticated);
        sessions_.SetPasscode(std::move(passcode));
        sessions_.SetConnectionAuthenticated(false);
...
bool TerminalHost::SendToPeer(const NetAddr& peer, std::span<const uint8_t> message) {
    return sock_ != nullptr && SendTermMessage(*sock_, peer, message);
}
...
void TerminalHost::HandleDatagram(const NetAddr& from, std::span<const uint8_t> datagram) {
    if (!Running()) return;
    ...
    std::vector<std::vector<uint8_t>> messages;
    {
        ...
        if (header->type == kTermRecordType) {
            deskhub::RecordStream& stream = streams_[from.Pack()];
            std::optional<std::vector<uint8_t>> first = FeedTermRecord(stream, datagram);
            if (!first) return;
            messages.push_back(std::move(*first));
            std::vector<uint8_t> more;
            while (stream.Next(more)) messages.push_back(std::move(more));
        } else {
            deskhub::RecordStream unused;
            if (std::optional<std::vector<uint8_t>> message = FeedTermRecord(unused, datagram))
                messages.push_back(std::move(*message));
        }
    }

    for (const std::vector<uint8_t>& message : messages) HandleMessage(from, message);
}
...
void TerminalHost::HandleMessage(const NetAddr& from, std::span<const uint8_t> message) {
    ...
    if (header->type == deskhub::MsgType::TermOpen) {
        ...
        full.pairingAuthenticated =
            peerAuthenticated_ && peerAuthenticated_(from.Pack());
